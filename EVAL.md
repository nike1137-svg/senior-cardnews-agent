...
| 지표 | 정의 | 결과 |
|---|---|---|
| 카드 생성률 | 카드 파일이 실제로 만들어진 실행 | **9/38 (24%)** |
| 완주율 | 마지막 단계까지 끝난 실행 | 4/38 (11%) |
| 사람 개입 횟수 | 실행당 질문 수 (적을수록 좋다) | 평균 **1.7회** (최소 0 / 최대 10) |
| 루프 반복 | 실행당 판단 횟수 | 평균 10.9회 |
| 도구 실패 | 전체 도구 호출 중 실패 | 26건 / 429건 |

⚠️ **이 「도구 실패」 숫자를 그대로 믿으면 안 된다.** 안에 검토 에이전트의 반려 9건과
단계 허용 목록이 막은 5건이 섞여 있다 — 둘 다 도구가 깨진 게 아니라
...
| `run-0b6e4dbef589` | `gemini-3.5-flash-lite` (폴백) | 28 | **7/7** | 고친 뒤 손 안 대고 한 번 더 (dry-run, 개입 9회) |

**완주 4건이 서로 다른 모델에서 나왔고 그중 둘은 제공자도 다르다.**
막힌 자리는 모델이 아니라 구조에 있었다는 뜻이다 — 실제로 고친 것 중 모델 쪽 문제는 하나도 없었다.
뒤의 두 실행은 설정이 `gemini-3.5-flash` 였지만 한도에 걸려 **lite 로 갈아탄 채로 완주했다.**
폴백이 품질 저하가 아니라 **끝까지 가게 해 준 장치**로 작동했다.

마지막 `run-0b6e4dbef589` 은 고친 뒤 **아무 손도 대지 않고 돌린 것**이다.
세 번 연속 실패하던 자리를 이제 연달아 통과한다 — 한 번의 성공이 우연이 아니었다는 근거다.
...
그 뒤 `run-0b6e4dbef589` 에서 같은 장치가 **다른 방식으로** 한 번 더 걸렸다.
이번에는 모델이 경로를 5개 줬는데 **이름이 실제 파일과 달랐다.**

```
모델이 준 경로(5개)와 실제 카드가 달라 폴더에서 직접 읽었다 (5장)
```

개수를 맞게 준 것과 맞는 파일을 준 것은 다르다. **모델이 틀리는 방식은 매번 새롭다** —
그래서 "이번 실수를 막는" 것이 아니라 **앱이 직접 읽는 쪽으로** 고친 것이 맞았다.
같은 실행에서 발송 잠금도 정상 작동해 dry-run 으로 처리됐다.

마지막 완주 실행이 실제로 무엇을 불렀는지 그대로 펼치면 이렇다.

```
단계1 조사      list_past_publications  ok    {"count": 3, "since": "2026-06-12", "publica
                web_search              ok    검색어='서울시 시니어 장례지원 사업' 최근 30일 → 10건 1. 경기 성남시
                web_search              ok    검색어='서울시 공영장례 지원 조례 어르신' 최근 30일 → 9건 1. 통합돌봄
                web_search              ok    검색어='서울시 공영장례 지원' 최근 30일 → 4건 1. 제주시, 무연고 사망
                ask_human               ok    질문: '서울시 시니어 장례지원 사업'에 대한 최근 30일 내 구체적인 최신 뉴
                finish_step             ok    '서울시 시니어 장례지원 사업'에 대한 최근 30일 내 구체적인 뉴스가 검색되지
단계2 후보 선택 ask_human               ok    질문: '1인 독거노인 복지혜택' 안에서 어떤 구체적인 소식을 카드뉴스 주제로 
                finish_step             ok    사용자가 카드뉴스 주제로 '서울시 고독사 예방 및 지원 사업'을 선택함
단계3 심층 검증 web_search              반려  web_search 은 이 단계에서 쓸 수 없다. 허용: fetch_articl
                ask_human               ok    질문: '서울시 고독사 예방 및 지원 사업'에 대한 최신 소식을 찾기 위해 웹 
                ask_human               ok    질문: 현재 '서울시 고독사 예방 및 지원 사업'에 대한 최근 30일 내 구체적
                ask_human               ok    질문: 현재 단계(심층 검증)에서는 웹 검색을 할 수 없습니다. 이미 확인된 소
                fetch_article           ok    원문 확인: '서울시 복지 분야별 사이트 메인' 게시일=2026-09-09 본문
                fetch_article           ok    원문 확인: '서울시 복지 분야별 사이트 메인' 게시일=2026-09-09 본문
                finish_step             ok    서울시 복지 메인 페이지 원문(2026-09-09)을 fetch_article로
단계4 스토리보드get_audience_profile    ok    {"who": "60~80대 시니어. 경로당·복지관·시니어 채널을 통해 안내를 
                ...
                ask_human               ok    질문: [스토리보드 초안]\n1장(표지): 서울시 어르신 복지 혜택\n2장: 어
                ask_human               ok    질문: 어떤 방향으로 스토리보드를 수정하시겠습니까?
                finish_step             ok    사용자의 피드백(금일 날짜 2026년 9월 10일 기준 서울시 복지 혜택 안내)
단계5 카드 합성 compose_cards           ok    카드 5장 생성·검증 완료 → …\output\run-0b6e4dbef589
                검토에이전트            ok    모든 검사 항목을 통과했습니다.
                finish_step             ok    승인된 스토리보드와 2026년 9월 10일 기준 서울시 복지 혜택 안내 내용을 
단계6 검수      ask_human               ok    질문: 완성된 카드뉴스를 확인해주세요. 2026년 9월 10일 기준 서울시 복지
                finish_step             ok    사용자로부터 완성된 카드뉴스 발행 승인을 받았습니다. 모든 단계를 성공적으로 완
단계7 발송      ask_human               ok    질문: 완성된 2026년 9월 10일 기준 서울시 복지 혜택 안내 카드뉴스를 L
                send_line               ok    모델이 준 경로(5개)와 실제 카드가 달라 폴더에서 직접 읽었다 (5장). [d
                record_publication      ok    {"ok": true, "id": 4, "published_at": "2026-
                finish_step             ok    사용자의 발송 요청에 따라 LINE 발송(dry-run)을 완료하고 발행 이력을
```
...
| 09-10 03:40 | 시니어 장례지원 사업 | `gemini-3.5-flash-lite` | 7/7 | ⭕ | 9 | 28 | 110,553 | list_past_publications → web_search → web_search → web_search → web_search✗ → fe |
...
| `gemini/gemini-3.5-flash-lite` | 16 | 3/16 (19%) | 1.3 | 11.6 | 5 | 30,468 |
...
| 실패 라벨 | 횟수 | 어느 단계에서 났나 |
|---|---|---|
| 도구오류 | 15 | 단계1 조사(8건), 단계3 심층 검증(5건), 단계7 발송(2건) |
| 사실오류 | 5 | 단계5 카드 합성(5건) |
...
| 도구오류 | `web_search` | 3 심층 검증 | 4 | web_search 은 이 단계에서 쓸 수 없다. 허용: fetch_article, get_weather |
...
- **`도구오류` 15건 중 5건은 고장이 아니라 방어가 작동한 것이다.**
...
- **검토 에이전트는 17번 판정해 9번 반려했다** (통과율 47%).
  만든 것과 같은 모델에게 보게 했는데도 반려가 났다 —
  역할과 프롬프트를 나눈 것만으로 잡힌다
- **시간 병목은 조사 한 곳이다.** **조사** 34/36건 (합 363초) · **심층 검증** 2/36건 (합 29초).
