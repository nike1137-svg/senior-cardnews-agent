...
```
실행 12건 · 카드 생성 2건 · 완주(7/7) 1건 · 커밋 24개
```

완주가 1건인 이유는 `EVAL.md` 1장에 원인 표로 적어뒀다. **실패한 실행을 지우지 않았다.**

### 이 기기 상태

| 있음 | `GEMINI_API_KEY` · `TAVILY_API_KEY` · LINE 토큰/수신자 · `agy` 1.1.28(로그인됨) · Pretendard |
|---|---|
| 없음 | `OPENAI_API_KEY` (없어도 전부 동작) |
...
- **종료 조건**을 둔다 (재시도 3회 / 루프 30회 / 작업시간 10분 / 이미지 8회).
  작업시간은 **사람 대기를 뺀 값**이고, 루프는 검토 재작업 때문에 20→30 으로 올렸다 (D-020)
