...
n8n 은 트리거만 담당하고 판단은 코드가 한다 (D-006). 붙이는 방법은 간단하다.

```
n8n Schedule 노드 (월요일 06:00)
  → HTTP Request 노드로 POST https://cardnews.dodami-ai.com/runs
     body: topic=이번 주 시니어 건강 정보, region=서울 노원구
  → 에이전트가 조사까지 하고 waiting_for_user 로 대기
  → 담당자가 출근해 화면에서 답만 하면 됨
```

앱 쪽은 **손댈 필요가 없다.** `POST /runs` 가 이미 폼 데이터를 받는다.
🔴 온담이·이음이 워크플로와 같은 인스턴스를 쓰더라도 **워크플로와 자격증명은 분리**할 것.

**카드 버전 롤백 ⭐확장6** — 재생성 전 카드를 남겨두고 되돌리기. 값이 가장 낮다.

### 3순위 — 알려진 약점 (여유가 있으면)

- **`fetch_article` 이 실제 실행에서 한 번도 안 쓰였다.** 도구 자체는 동작을 확인했지만
  (정부 사이트에서 본문 4000자 추출 성공), 에이전트가 스스로 고른 적은 없다.
  심층 검증 단계에서 URL 을 넘겨주는 방식으로 유도하면 채워진다
- 평가 세트가 얇다. 같은 시나리오를 3회 이상 반복 측정한 데이터가 없다
- 테스트 코드가 없다
